...
@pytest.mark.asyncio
async def test_generation_defers_while_internal_pipeline_running():
    """We must not start generating until the internal pipeline has finished."""
    gitlab = AsyncMock()
    gitlab.get_mr_pipeline.return_value = {"id": 9, "status": "running"}
    ...
    # Generation was NOT invoked, and the commit stays under watch for a retry.
    proc.assert_not_awaited()
    assert (1, "abc1234") in main._pending_watches


@pytest.mark.asyncio
async def test_generation_runs_once_internal_pipeline_succeeded():
    gitlab = AsyncMock()
    gitlab.get_mr_pipeline.return_value = {"id": 9, "status": "success"}
