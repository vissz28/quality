...
## Purpose
The Test Calibrator works from diffs and titles, which gives it limited context about what the code actually does. The Software Engineer fills that gap: it reads the code as an engineer would, identifies what matters, and hands the Test Calibrator a precise brief so generated tests are accurate and meaningful rather than generic.
...
GitLab Webhook
      │
      ▼
Software Engineer      ← analyses code, produces structured brief
      │
      ▼
Test Calibrator        ← uses brief as context to generate Gherkin + Playwright
```
